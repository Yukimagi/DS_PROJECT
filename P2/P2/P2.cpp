...
DataFrame arr[1000000], unionData[1000000], unionData2[1000000], unionData3[1000000], unionData4[1000000];
int arr_size, union_size, union_size2, union_size3, union_size4;
double price;
int main()
{ //宣告一個檔案輸入流fin，開啟TWII_withRepeatedData.csv。
    ...
    if (fin.is_open())
    {
        ...
        while (getline(fin, LINE))
        {
            ...
            arr_size++;
            unionData3[union_size3].setDate(date);
            unionData3[union_size3].setencode(c2 + "_" + c3 + "_" + c4 + "_" + c5);
            unionData3[union_size3].setencode2(date + "_" + c6);
            unionData3[union_size3].setdeal_price(price);
            union_size3++;
        }
    }
    fin.close();
    ...
    //第2題，使用二元搜尋，確定商品TXO_1000_201706_P是否存在這個資料集中
    DataFrame target;
    ...
    //第3題，使用二元搜尋，確定商品TXO_9500_201706_C是否存在這個資料集中
    target.setencode("TXO_9500_201706_C");
    index = BinarySearch(unionData, -1, union_size, target);
    if (index != union_size && target.getencode() == unionData[index].getencode())fout << "存在 就在unionData[" << index << "]" << endl;
    else                                                                        fout << "不存在" << endl;

    //第4題，使用二元搜尋，確定商品GIO_5500_201706_C是否存在這個資料集中 
    ...
    for (int i = 0; i < 10; i++)fout << unionData2[i].getencode2() << " " << unionData2[i].getdeal_price() << " " << endl;
    ...
    for (int i = 0; i < union_size3; i++) {
        if (unionData3[i].getencode() == "TXO_9900_201705_C") {
            ...
            //fout << unionData3[i].getencode2() <<" price: "<< unionData3[i].getdeal_price() << endl;
        }
    }
    //for (int i = 0; i < union_size4; i++)fout << unionData4[i].getDate() << " " << unionData4[i].getencode() << " " << unionData4[i].getdeal_price() << unionData4[i].getencode2() << endl;
    ...
    for (int i = 1; i < union_size4; i++)
    {
        ...
        if (RETURN > MAX)
        {
            ...
            Pt_check_big = Pt;
            Pt_1_check_big = Pt_1;
            //fout << "第" << i << "天的 tick-based是 " << unionData4[i].getencode2() << endl;
            //fout << "第" << i - 1 << "天的 tick-based是 " << unionData4[i - 1].getencode2() << endl;
        }
        if (RETURN < MIN)
        {
            MIN = RETURN;
    ...
    fout << "maximum return 為" << MAX << " " << "Pt date=" << unionData4[MAXINDEX].getencode2() << " " << "price" << " " << Pt_check_big << " " << "Pt-1 date=" << unionData4[MAXINDEX - 1].getencode2() << "price" << " " << Pt_1_check_big << endl;
    fout << "minimum return 為" << MIN << " " << "Pt date=" << unionData4[MININDEX].getencode2() << " " << "price" << " " << Pt_check_small << " " << "Pt-1 date=" << unionData4[MININDEX - 1].getencode2() << "price" << " " << Pt_1_check_small << endl;
    fout.close();
}